...
async function seedData() {
  try {
    ...
    await db.query("DELETE FROM hq_reports");
    ...
    const branches = [
      ...
      { name: "Mumbai Metro", loc: "Andheri East" },
      { name: "Delhi Depot", loc: "Connaught Place" },
      { name: "Hyderabad Heights", loc: "Banjara Hills" },
      { name: "Pune Pavilion", loc: "Koregaon Park" },
      { name: "Ahmedabad Alley", loc: "SG Highway" },
      { name: "Kolkata Kiosk", loc: "Park Street" }
    ];
    ...
    for (const bData of branches) {
      ...
      // 8. Seed Sales (Last 90 days for better ML Trajectory)
      const products = ["Burger", "Pizza", "Coffee", "Pasta"];
      for (let i = 0; i < 90; i++) {
        const date = new Date();
        date.setDate(date.getDate() - i);
        
        // Simulating a 15-20% growth trend over 90 days
        const growthFactor = 1.0 + ((89 - i) * 0.002); 
        const numSales = Math.floor((4 + Math.random() * 8) * growthFactor);
        ...
        for (let s = 0; s < numSales; s++) {
          await db.query(
            ...
            [
              ...
              "Regular Customer", 
              (200 + Math.random() * 800) * growthFactor, 
